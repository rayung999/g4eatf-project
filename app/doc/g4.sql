...
CREATE TABLE event (
  ...
  pub       BOOLEAN      NOT NULL DEFAULT 0 COMMENT '공개여부', -- 공개여부
  datetime  DATETIME     NOT NULL COMMENT 'createdate', -- createdate
  thumbnail VARCHAR(250) NULL     COMMENT 'thumbnail' -- thumbnail
)
COMMENT '이벤트';
...
CREATE TABLE feed_file (
  ...
  fno        INTEGER      NOT NULL COMMENT '피드번호', -- 피드번호
  filename   VARCHAR(200) NOT NULL COMMENT '첨부파일명', -- 첨부파일명
  path       VARCHAR(200) NULL     COMMENT '파일 경로', -- 파일 경로
  savename   VARCHAR(200) NOT NULL COMMENT '저장파일명' -- 저장파일명
)
COMMENT '피드 첨부파일';
...
CREATE TABLE party_file (
  ...
  filename VARCHAR(200) NULL     COMMENT '첨부파일명', -- 첨부파일명
  path     VARCHAR(200) NOT NULL COMMENT '파일 경로', -- 파일 경로
  savename VARCHAR(200) NULL     COMMENT '저장파일명' -- 저장파일명
)
COMMENT '파티게시판 첨부파일';
...
CREATE TABLE event_file (
  efileno  INTEGER      NOT NULL COMMENT '첨부파일 번호', -- 첨부파일 번호
  eno      INTEGER      NOT NULL COMMENT '게시글 번호', -- 게시글 번호
  filename VARCHAR(200) NOT NULL COMMENT '첨부파일명', -- 첨부파일명
  path     VARCHAR(200) NULL     COMMENT '파일 경로', -- 파일 경로
  savename VARCHAR(200) NOT NULL COMMENT '저장파일명' -- 저장파일명
...
ALTER TABLE event_file
  ADD CONSTRAINT PK_event_file -- 이벤트 첨부파일 기본키
    PRIMARY KEY (
      efileno -- 첨부파일 번호
    );
...
-- 이벤트 첨부파일
ALTER TABLE event_file
  ADD CONSTRAINT FK_event_TO_event_file -- 이벤트 -> 이벤트 첨부파일
    FOREIGN KEY (
      eno -- 게시글 번호
    )
    REFERENCES event ( -- 이벤트
